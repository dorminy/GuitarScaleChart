...
#   print("Tried to reset!")
    for i in range(0, 25):
        for gss in range(0, 6):
            Label(chartgui,borderwidth=2, relief="groove", width=fretsizes[i], text=getnotename(i + offsetArray[gss])).grid(row=gss + 2, column=i + 1, padx=0, pady=0)
    ...
    for i in range(0, 25):
        for gss in range(0, 6):
            ...
            if ourtonic == getnotename(i + start % 12):
                Label(chartgui,borderwidth=2, relief="groove", width=fretsizes[i], text=getnotename(i + start), bg='spring green').grid(row=gss + 2, column=i + 1, padx=0, pady=0)
            # draw yellow for notes in the scale
            elif getnotename(i + start) in ournotes:
                Label(chartgui,borderwidth=2, relief="groove", width=fretsizes[i], text=getnotename(i + start), bg='medium spring green').grid(row=gss + 2, column=i + 1, padx=0, pady=0)
            # only write notename
            else:
                Label(chartgui,borderwidth=2, relief="groove", width=fretsizes[i], text=getnotename(i + start)).grid(row=gss + 2, column=i + 1, padx=0, pady=0)

if __name__ == "__main__":
    chartgui.geometry('800x280+400+300')
    chartgui.title('GuitarScaleChart - Colin Burke, 2017')
    chartgui.configure(background='white')
    ourx = 40
    oury = 20
    # For our fret (column) labels on top and bottom
    for i in range(0, 25):
        Label(chartgui, text=i, font='helvetica', background='white').grid(row=0, column=i + 1, padx=0, pady=10)
        Label(chartgui, text=i, font='helvetica', background='white').grid(row=9, column=i + 1, padx=0, pady=10)

    # For our string (row) labels
    stringarray = ['E', 'B', 'G', 'D', 'A', 'E']
    for gss in range(0, 6):
        Label(chartgui, borderwidth=2, relief="groove", width=fretsizes[i] , text=stringarray[gss], font="comicsans").grid(row=gss + 2, column=0, padx=10, pady=0)
    #print(ourscale)
    # draw our whole scale
    applyit("")

    keymenu = OptionMenu(
        chartgui, variable, 'E ', 'F ', 'F#', 'G ', 'Ab', 'A ', 'Bb', 'B ', 'C ', 'Db', 'D ', 'Eb', command=applyit).place(x=ourx * 2, y=oury * 12)

    scalemenu = OptionMenu(chartgui, variable2, *scales.keys(), command=applyit).place(x=ourx * 4, y=oury * 12)
